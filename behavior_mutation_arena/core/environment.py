...
class ArenaEnvironment:
    ...
    def _spawn_agents(self) -> None:
        all_cells = self.rng.permutation(self.grid_size * self.grid_size)
        agent_cells = all_cells[: self.population_size]
        for agent_id, flat_index in enumerate(agent_cells):
            x, y = divmod(int(flat_index), self.grid_size)
    ...
    def _spawn_item(self, item_kind: ItemKind) -> None:
        candidates = np.argwhere((self.occupancy == -1) & (self.item_grid == ItemKind.EMPTY))
        if len(candidates) == 0:
            return
